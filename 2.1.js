...
const regexMatch = /(\d+)-(\d+)\s(.):\s(.*)$/;
const processed = input.trim().split('\n').map(line => {
  const [, min, max, char, toTest] = line.match(regexMatch);
  const countMatch = toTest.match(new RegExp(`${char}`, 'g'));

  const count = 0 || countMatch && countMatch.length;
  const pass = (count >= min) && (count <= max);

  return pass;
});

const passed = processed.reduce((acc, t) => {
  if (t) return acc + 1;
  return acc;
}, 0);
